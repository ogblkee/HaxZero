document.head.appendChild(Object.assign(document.createElement("style"), { innerHTML: "#thumb,body{touch-action:none}body{user-select:none;height:100%}@media only screen and (max-device-width:480px){#thumb,#joystick,#kick{zoom:60%}}" }));
document.querySelector('.gameframe').contentWindow.document.head.appendChild(Object.assign(document.createElement("style"), { innerHTML: ".room-view,.roomlist-view{height:100%;margin-top:0}.game-view{height:100%;margin-top:0}.choose-nickname-view{height:100%;margin-top:0}.settings-view{height:100%;margin-top:0}.create-room-view{height:100%;margin-top:0}" }));
...
const constrolsStyleBase = "#joystick,#kick{z-index:100;bottom:CONTROLS_MARGINvw}.neo{opacity:CONTROLS_OPACITY;background-color:#c2c2c255;box-shadow:6px 6px 10px 0 #a5abb133,-5px -5px 9px 0 #a5abb133;border-radius:50%;padding:CONTROLS_WIDTHvw;position:fixed;width:CONTROLS_WIDTHvw;height:CONTROLS_WIDTHvw}#kick{right:CONTROLS_MARGINvw;border:none;background-color:#fff;cursor:pointer}#kick{opacity:KICK_OPACITY}#thumb{width:40%;height:40%;border-radius:50%;box-shadow:0 0 5px 0 #0000004d;background-color:#0000001f;position:absolute;left:30%;top:30%}#joystick{left:CONTROLS_MARGINvw}[view=hidden]{display:none}[view=visible]{display:block}";
...
function init() {
    ...
    aboutHandler.style.cssText = 'background: #1a2125; position: absolute; width: 100%; height: 100%; display: none; justify-content: center; flex-direction: column; align-items: center; margin: 0; z-index: 9999;';
    aboutHandler.innerHTML = '<div class="dialog basic-dialog" style="max-width: 50%;"><h1>About us</h1><p>We are Vixel Dev, a small development studio that wants the Haxball community to grow, we managed to created a optimized mobile version with controls and some extra features.</p><button data-hook="closeabout">Close</button></div>';
...
function createStoreButton() {
    ...
    store.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" xml:space="preserve" viewBox="0 0 407 407" fill="white" style="height:0.85em; width: auto"><path d="M402 84 323 5c-3-3-7-5-12-5H17C8 0 0 8 0 17v373c0 9 8 17 17 17h373c9 0 17-8 17-17V96c0-5-2-9-5-12zm-46-31v52H52V53h304zM34 85h339v297H34z"/></svg>';
...
function setupGameUI() {
    ...
    if (!getByDataHook('chat-toggle')) {
        ...
        button.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 20 20"><path fill="white" d="M5.8 12.2V6H2C.9 6 0 6.9 0 8v6c0 1.1.9 2 2 2h1v3l3-3h5c1.1 0 2-.9 2-2V8c0-1.1-.9-2-2-2H5.8z"/></svg>';
...
function updateControlsOptions(w, m, o, f = false) {
    ...
    controlsHandler.innerHTML = constrolsStyleBase.replace(/CONTROLS_WIDTH/g, w.toString()).replace(/CONTROLS_MARGIN/g, m.toString()).replace(/CONTROLS_OPACITY/g, o.toString()).replace(/KICK_OPACITY/g, o.toString());
...
function setupControls() {
    ...
    inputOptionsHandler.innerHTML = '<div class="dialog settings-view" style="height:min-content"><h1>Controls</h1><button data-hook="closeinput" style="position:absolute;top:12px;right:10px">Back</button><div class="option-row" style="margin: 10px 0"><label>Width</label><span>20</span><input type="range" min="10" max="30" value="20" step="1"></div><div class="option-row" style="margin: 10px 0"><label>Margin</label><span>5</span><input type="range" min="0" max="20" value="5" step="1"></div><div class="option-row" style="margin: 10px 0"><label>Opacity</label><span>1</span><input type="range" min="0.1" max="1" value="1" step="0.1"></div><button data-hook="resetinput" style="margin-top: 15px; width: 100%">Reset to default</button></div>';
